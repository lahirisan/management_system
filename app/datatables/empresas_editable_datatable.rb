#encoding: UTF-8
class EmpresasEditableDatatable 
  delegate :params, :h, :link_to, :content_tag, :empresa_path, :edit_empresa_path, :empresa_productos_path, :empresa_glns_path, :empresa_etiqueta_path,  to: :@view
...
private
  ...
  def data
    ...
    empresas.map do |empresa|
        ...
        estatus_administrativo = (empresa.solv == 2) ? "DEUDOR" : "SOLVENTE" # SE VERIFICA EL ESTATUS ADMINISTRATIVO DE LA EMPRESA
        ...
        if estatus_administrativo == "SOLVENTE" 
          ...
          [ 
            ...
            link_to(( content_tag(:span, '',:class => 'ui-icon ui-icon-extlink')+'Editar').html_safe, edit_empresa_path(empresa.prefijo), {:class => "ui-state-default ui-corner-all botones_servicio", :title => "Editar la empresa #{empresa.nombre_empresa}"}),
            boton_productos,
            ...
            link_to(( content_tag(:span, '',:class => 'ui-icon ui-icon-extlink')+'Asociar_Prefijo').html_safe, "/empresas/#{empresa.prefijo}/edit?asociar_prefijo=true", {:class => "ui-state-default ui-corner-all botones_servicio", :title => "Asociar prefijo a empresa existente"})
          ]
