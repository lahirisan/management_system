class EmpresasRetiradasDatatable 
...
private
  ...
  def data
    ...
    empresas.map do |empresa|
      ...
      [ 
        ...
        link_to(( content_tag(:span, '',:class => 'ui-icon ui-icon-extlink')+'Servicios').html_safe, "/empresas/#{empresa.prefijo}/empresa_servicios?empresa_retirada=true", {:class => "ui-state-default ui-corner-all botones_servicio", :title => "Servicios asociados a la empresa #{empresa.nombre_empresa}"}),
  ...
  def fetch_empresas
    ...
    empresas = empresas.where("empresa.prefijo like :search or empresa.nombre_empresa like :search or empresa_clasificacion.descripcion like :search or ciudad.nombre like :search or tipo_usuario_empresa.descripcion like :search or empresa.fecha_retiro like :search or motivo_retiro.descripcion like :search ", search: "%#{params[:sSearch]}%") if params[:sSearch].present? # Filtro de busqueda general
    
    empresas = empresas.where("empresa.prefijo like :search0", search0: "%#{params[:sSearch_0]}%" ) if params[:sSearch_0].present? 
    empresas = empresas.where("empresa.nombre_empresa like :search1", search1: "%#{params[:sSearch_1]}%" )   if params[:sSearch_1].present? # Filtro nombre empresa
  ...
  def sort_column
     ...
     columns = %w[empresa.prefijo empresa.nombre_empresa empresa_clasificacion.descripcion ciudad.nombre tipo_usuario_empresa.descripcion empresa.fecha_retiro motivo_retiro.descripcion]
