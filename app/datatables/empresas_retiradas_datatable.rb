  ...
  def data
    ...
    empresas.map do |empresa|
      ...
      fecha_retiro = empresa.try(:fecha_retiro).nil? ? '' : empresa.fecha_retiro.strftime("%Y-%m-%d")
        ...
        link_to(( content_tag(:span, '',:class => 'ui-icon ui-icon-extlink')+'Carta_Retiro').html_safe, "/empresas/#{empresa.prefijo}.pdf?retiro_individual=true", {:class => "ui-state-default ui-corner-all botones_servicio", :title => "Etiqueta de la empresa #{empresa.nombre_empresa}"})
