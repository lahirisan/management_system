...
private
  ...
  def fetch_empresas
    ...
    if params[:sSearch_6].present?
      empresas = empresas.where("CONVERT(varchar(255),  empresa.fecha_retiro ,126) like :search6", search6: "%#{params[:sSearch_6]}%")
    end

    if params[:sSearch_7].present?
      empresas = empresas.where("motivo_retiro.descripcion like :search7", search7: "%#{params[:sSearch_7]}%")
    end
  ...
  def sort_column
     ...
     columns = %w[nil empresa.prefijo empresa.nombre_empresa empresa.fecha_activacion  ciudad.nombre empresa.rif empresa.fecha_retiro motivo_retiro.descripcion]
