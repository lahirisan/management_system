...
if params[:tipo_gln] != ''
  @glns = @glns.where("tipo_gln.nombre like :search", search1: "%#{params[:tipo_gln]}%" )
end
...
glns = Array.new
glns = [["Prefijo", "Nombre Empresa", "Fecha Inscripción", "Direcccion Empresa", "Estado", "Ciudad", "RIF", "Estatus"]]

@glns.each do |empresa|

  glns << [empresa_gln.try(:gln).try(:gln), empresa_gln.try(:gln).try(:tipo_gln).try(:nombre), empresa_gln.try(:gln).try(:codigo_localizacion), empresa_gln.try(:gln).try(:descripcion), empresa_gln.try(:gln).try(:estatus).try(:descripcion), empresa_gln.try(:gln).try(:fecha_asignacion).strftime("%Y-%m-%d"), empresa_gln.try(:gln).try(:estado).try(:nombre),empresa_gln.try(:gln).try(:municipio).try(:nombre), empresa_gln.try(:gln).try(:ciudad).try(:nombre)] 
